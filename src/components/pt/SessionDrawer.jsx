...
export default function SessionDrawer({ open, onClose, eventData, onChanged }) {
  // eventData: { session, start, end } – truyền vào từ handleEventClick
  ...
  const studentAvatar = session?.student?.avatar || 'https://placehold.co/100x100?text=Avatar'
  const pkgName = eventData?.sessionPackageName || '' // nếu có
  const startStr = eventData?.start ? new Date(eventData.start).toLocaleString() : ''
  const endStr = eventData?.end ? new Date(eventData.end).toLocaleString() : ''
      ...
      >
        ...
        <div className="h-[calc(100vh-56px)] overflow-y-auto p-4 space-y-4">
          ...
          <div className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 p-3">
            <img src={studentAvatar} className="h-12 w-12 rounded-full object-cover" />
            <div>
              <div className="text-white font-medium">{studentName}</div>
              {pkgName && <div className="text-xs text-gray-400">{pkgName}</div>}
            </div>
          </div>
          ...
          <div className="rounded-xl border border-white/10 bg-white/5 p-3">
            ...
            <div className="mt-3 grid grid-cols-1 gap-2 text-sm text-gray-300">
              <div><span className="text-gray-400">Bắt đầu: </span>{startStr}</div>
              <div><span className="text-gray-400">Kết thúc: </span>{endStr}</div>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-white/10 bg-white/5 p-3">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Badge active={session?.status === 'rescheduled'}>rescheduled</Badge>
              <Badge active={session?.status === 'cancelled'}>cancelled</Badge>
            </div>

            <div className="mt-3 flex flex-wrap gap-2">
              <ActionBtn disabled={loading} onClick={() => setStatus('completed', 'present')}>Đánh dấu Completed</ActionBtn>
              <ActionBtn disabled={loading} onClick={() => setStatus('missed', 'absent')}>Đánh dấu Missed</ActionBtn>
              <ActionBtn disabled={loading} onClick={() => setStatus('cancelled')}>Huỷ buổi</ActionBtn>
              <button
          ...
          {!!session?.ptNote && (
            <div className="rounded-xl border border-white/10 bg-white/5 p-3">
              ...
              <div className="text-gray-200 text-sm whitespace-pre-wrap">{session.ptNote}</div>
            </div>
          )}
          {!!session?.studentNote && (
            <div className="rounded-xl border border-white/10 bg-white/5 p-3">
              <div className="mb-2 text-sm text-gray-400">Ghi chú của học viên</div>
              <div className="text-gray-200 text-sm whitespace-pre-wrap">{session.studentNote}</div>
            </div>
          )}
        </div>
      </aside>
...
function Badge({ children, active }) {
  return (
    <span className={`rounded-full px-2.5 py-1 text-xs ${
      active ? 'bg-orange-500/30 text-orange-200' : 'bg-white/10 text-gray-300'
    }`}>{children}</span>
  )
}
